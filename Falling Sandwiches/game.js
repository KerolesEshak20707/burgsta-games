...
class GameScene extends Phaser.Scene {
    ...
    actualSpawnItems() {
        ...
        if (this.gameManager.discount < 5) {
            ...
            numItems = Math.floor(Math.random() * 2) + 4; // 4-5 ساندوتشات (كمية معقولة)
        } else if (this.gameManager.discount < 10) {
            badChance = 0.50; // 50% سيئة
            goodChance = 0.50; // 50% جيدة
            numItems = Math.floor(Math.random() * 3) + 3; // 3-5 ساندوتشات 
            ...
            badChance = Math.min(0.85, 0.60 + (currentDifficulty * 0.08)); // من 60% إلى 85% سيئة! 🔥
